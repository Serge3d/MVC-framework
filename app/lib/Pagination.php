...
class Pagination {
    ...
    private $limit;
    // $link - ссылка на страницу, на которой используется пагинация
    private $link;

    public function __construct($page, $total, $link, $limit = 10) {
        $this->page = $page;
        $this->total = $total;
        $this->link = $link;
        $this->limit = $limit;
    ...
    public function get() {
        ...
        if (!is_null($links)) {
            if ($this->current_page > 1) {
                ...
                $links .= $this->generateHtml(1, 'Назад');
            }
            if ($this->current_page < $this->amount) {
                //если мы не на последней странице, добавить кнопку назад                
                $links = $this->generateHtml($this->amount, 'Вперед').$links;
            }
        }
        $html .= $links.' </ul></nav>';
    ...
    private function generateHtml($page, $text = null) {
        ...
        //</li>
        return '<li class="page-item"><a class="page-link" href="/'.$this->link.'/'.$page.'">'.$text.'</a></li>';
    }
